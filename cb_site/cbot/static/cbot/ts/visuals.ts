...
class TextTyper {
    ...
    public type(htmlElem: JQuery<HTMLElement>, targetText: string, timeSpanMS: number) {
    ...
    public async typeText(htmlElem: JQuery<HTMLElement>, targetText: string, timeSpanMS: number): Promise<void> {
        // don't know how effective for long chunks of text, but this should be A okay for now
        const textArray: string[] = targetText.split(" ");
        const arrLength = textArray.length;
